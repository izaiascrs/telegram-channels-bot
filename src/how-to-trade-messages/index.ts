import { LocalPath, MessageLike } from 'telegram/define';

type THowToTradeMessage = {
  message: MessageLike,
  file?: LocalPath
}

export const howToTradeMessages: THowToTradeMessage[] = [
	{
		message: '🤑👇👇👇\n' +
    '\n' +
    '📌🗒️ **COMO APROVEITAR MELHOR NOSSOS SINAIS TOP:**\n'  +
    '\n' +
    '\n' +
    '🚨 NÃO RECOMENDAMOS O USO DE MARTINGALE, PORÉM, SE VOCÊ DECIDIR USAR, SUGERIMOS QUE UTILIZE SOMENTE 1 MARTINGALE (SE NECESSÁRIO) 🚨\n' +
    '\n' +
    '\n' +
    '⚠ 👉 🔎 FILTRE AS OPERAÇÕES: \n' +
    '\n' +
    '✔ Verifique a tendência e entre somente a favor da tendência.\n' +
    '\n' +
    '❌ Não entre em todas as operações!\n' +
    '\n' +
    '👉 Dê preferência para entrar nas operações que aconteceram um GAP em seu favor. Se ainda não sabe o que é GAP, assista o vídeo em nosso Canal do Youtube na playlist da QUOTEX.\n' +
    '\n' +
    '👉 Fique atento ao Tempo de Expiração da operação ⏰\n' +
    '\n' +
    '\n' +
    '🚥 Como pegar os sinais catalogados?\n' +
    '\n' +
    'Assista o vídeo em nosso Canal do Youtube na playlist da QUOTEX: https://youtu.be/HJi_NQsuuK0\n' +
    '\n' +
    '\n' +
    '👉 Siga um Gerenciamento inteligente e profissional.\n' +
    '\n' +
    '\n' +
    '👉 Não opere se estiver com o seu psicológico abalado de alguma forma.\n' +
    '\n' +
    '\n' +
    '👉 Não opere colocando em risco com dinheiro comprometido com contas e sua subsistência. 🚫\n' +
    '\n' +
    '\n' +
    '💲👉 Se precisa saber mais sobre trade entre em contato com o nosso suporte e solicite materiais GRATUITOS 🆓',
	},
	{
		message: '🤑📌 **TOP DE SINAIS CATALOGADOS LOGO MAIS.... VAI PERDER?!** 🗒️👇👇👇\n'  +
    '\n' +
    '🚥 Como pegar os sinais catalogados?\n' +
    '\n' +
    '👉 Assista o vídeo em nosso Canal do Youtube na playlist da QUOTEX: https://youtu.be/HJi_NQsuuK0\n' +
    '\n' +
    '👉 Abra uma conta na Corretora AGORA com o nosso código para ter benefícios especiais: Crie AGORA GRATUITAMENTE uma conta com o nosso link para ganhar R$ 50.000 reais ou $ 10.000 dólares e treinar como e BÔNUS. 🤑👇 (https://broker-qx.pro/sign-up/fast/?lid=460583)  \n' +
    '\n' +
    '\n' +
    '🆘 Precisa de Ajuda? Chama o SUPORTE: https://wa.me/message/RMWFMXEKWKD3B1',
	}
];